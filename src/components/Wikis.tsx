...
import WikiArrowCard from "@components/WikiArrowCard"
import { cn } from "@lib/utils"

type Props = {
  tags: string[]
  data: CollectionEntry<"wikis">[]
}

export default function Wikis({ data, tags }: Props) {
  const [filter, setFilter] = createSignal(new Set<string>())
  const [wikis, setWikis] = createSignal<CollectionEntry<"wikis">[]>([])

  createEffect(() => {
    setWikis(data.filter((entry) => 
                ...
                <li>
                  <button onClick={() => toggleTag(tag)} class={cn("w-full px-2 py-1 rounded", "whitespace-nowrap overflow-hidden overflow-ellipsis", "flex gap-2 items-center", "bg-black/5 dark:bg-white/10", "hover:bg-black/10 hover:dark:bg-white/15", "transition-colors duration-300 ease-in-out", filter().has(tag) && "text-black dark:text-white")}>
                    <svg class={cn("size-5 fill-black/50 dark:fill-white/50", "transition-colors duration-300 ease-in-out", filter().has(tag) && "fill-black dark:fill-white")}>
                      <use href={`/assets/svg/ui.svg#square`} class={cn(!filter().has(tag) ? "block" : "hidden")} />
                      <use href={`/assets/svg/ui.svg#square-check`} class={cn(filter().has(tag) ? "block" : "hidden")} />
                    </svg>
                    {tag}
                  </button>
                </li>
      ...
      <div class="col-span-3 sm:col-span-2">
        <div class="flex flex-col">
          <div class="text-sm uppercase mb-2">
            MOSTRANDO {wikis().length} de {data.length} WIKIS
          </div>
          <ul class="flex flex-col gap-3">
            {wikis().map((wiki) => (
              <li>
                <WikiArrowCard entry={wiki} />
              </li>
            ))}
          </ul>
        </div>
