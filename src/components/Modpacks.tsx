...
type Props = {
  tags: string[]
  data: CollectionEntry<"modpacks">[]
}

export default function Modpacks({ data, tags }: Props) {
  const [filter, setFilter] = createSignal(new Set<string>())
  const [modpacks, setModpacks] = createSignal<CollectionEntry<"modpacks">[]>([])

  createEffect(() => {
    setModpacks(data.filter((entry) => 
      ...
      <div class="col-span-3 sm:col-span-2">
        <div class="flex flex-col">
          <div class="text-sm uppercase mb-2">
            MOSTRANDO {modpacks().length} de {data.length} Mods
          </div>
